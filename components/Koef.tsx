		...
		<>
			<div key={koef.id} className='flex flex-row w-full px-2 py-2 border border-t-0 text-sm -z-1 bg-gray-50'>
				<div className='w-6/12 my-auto flex-row flex'>
					<input onChange={(e) => handleKoefChange(koef.id, pos_id, koef.koef_code, koef.is_balancer, e.target.value as string)} 
						name='name' 
				...
				<div className='w-2/12 my-auto mr-0'>
					<input onChange={(e) => handleKoefChange(koef.id, pos_id, koef.koef_code, koef.is_balancer, parseFloat(e.target.value) as number)}
						name='value' 
